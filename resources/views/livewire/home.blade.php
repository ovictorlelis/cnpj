<div class="max-w-lg mx-auto p-4">
    <div class="space-y-4">
        <div>
            <input class="w-full mt-2 p-3 border border-zinc-300 focus:outline-none rounded-lg" id="cnpj" placeholder="00.000.000/0000-00" type="text" wire:model="cnpj" x-mask="99.999.999/9999-99" />
            <button class="mt-2 w-full bg-[#2c3e50] hover:bg-[#34495e] text-white p-3 rounded-lg" wire:click="consultarCnpj" ">
        ...
        <div class="bg-white p-4 border border-zinc-300 rounded-lg">
            ...
            <div class="mt-4">
                ...
                <p class="p-2"><strong>Entrada:</strong> {{ date( 'd/m/Y' , strtotime($item['data_entrada_sociedade']))}}</p>
            </div>
        </div>
        @endforeach
        <div class="bg-white p-4 border border-zinc-300 rounded-lg">
            ...
            <div class="mt-4">
                ...
                <p class="border-b p-2"><strong>Data início:</strong> {{ date( 'd/m/Y' , strtotime($dados['data_inicio_atividade']))}}</p>
                <p class="border-b p-2"><strong>Situação:</strong> {{ $dados['descricao_situacao_cadastral'] }}</p>
                <p class="p-2"><strong>Data situação:</strong> {{ date( 'd/m/Y' , strtotime($dados['data_situacao_cadastral']))}}</p>
            </div>
        </div>
        <div class="bg-white p-4 border border-zinc-300 rounded-lg">
